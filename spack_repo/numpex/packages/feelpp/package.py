...
class Feelpp(CMakePackage, CudaPackage, ROCmPackage):
    ...
    depends_on("petsc +mumps+hwloc+ptscotch +suite-sparse+hdf5 +hypre~kokkos")
    depends_on("slepc")
    depends_on("parmmg")

    depends_on('fmt+shared cxxstd=17', when='cxxstd=17')
    depends_on('fmt+shared cxxstd=20', when='cxxstd=20')
    depends_on('fmt+shared cxxstd=23', when='cxxstd=23')
    depends_on("cln")
    depends_on("ginac")
    depends_on("eigen")
    ...
    depends_on("gl2ps")
    depends_on("ruby")
    depends_on("kokkos +openmp+hwloc", when="+kokkos")
    depends_on("kokkos-kernels", when="+kokkos")
    depends_on("scorep", when="analysisTool=scorep")

    for cuda_arch in CudaPackage.cuda_arch_values:
        depends_on(
            "kokkos +openmp+hwloc+cuda+cuda_lambda cuda_arch=%s" % cuda_arch,
    ...
    for rocm_arch in ROCmPackage.amdgpu_targets:
        depends_on(
            "kokkos +openmp+hwloc+rocm amdgpu_target=%s" % rocm_arch,
